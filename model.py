...
class My_anchor_Loss(nn.Module):
    ...
    def forward(self, feat, label, cuda):
        ...
        for v in range(self.num_view):
            z = feat[v]
            centers_batch = (self.anchor[v].cuda()).index_select(0, label.long())
...
class Model(nn.Module):
    ...
    def __init__(self, base_model, D_m, D_g, D_p, D_e, D_h, D_a, graph_hidden_size, n_speakers, max_seq_len,
                 window_past, window_future, device,
        ...
        if self.graph_type == 'hyper':
            self.graph_model = HyperGCN(a_dim=D_g, v_dim=D_g, l_dim=D_g, n_dim=D_g, nlayers=64,
                                        ...
                                        return_feature=self.return_feature, use_residue=self.use_residue, device=device,
        ...
        if self.multi_modal:
            ...
            if self.att_type == 'concat_subsequently':
                ...
                if self.use_residue:
                    ...
                    self.smax_fc = nn.Linear((graph_hidden_size*2)*len(self.modals), n_classes) # 3072, 6
    ...
    def forward(self, U, qmask, umask, seq_lengths, U_a=None, U_v=None, epoch=None):
        ...
        if self.norm_strategy == 'LN':
            r1 = self.normLNa(r1.transpose(0, 1).reshape(-1, feature_dim)).reshape(-1, seq_len, feature_dim).transpose(
                1, 0)
            r2 = self.normLNb(r2.transpose(0, 1).reshape(-1, feature_dim)).reshape(-1, seq_len, feature_dim).transpose(
                1, 0)
            r3 = self.normLNc(r3.transpose(0, 1).reshape(-1, feature_dim)).reshape(-1, seq_len, feature_dim).transpose(
                1, 0)
            r4 = self.normLNd(r4.transpose(0, 1).reshape(-1, feature_dim)).reshape(-1, seq_len, feature_dim).transpose(
                1, 0)
        elif self.norm_strategy == 'BN':
            r1 = self.normBNa(r1.transpose(0, 1).reshape(-1, feature_dim)).reshape(-1, seq_len, feature_dim).transpose(
                1, 0)
            r2 = self.normBNb(r2.transpose(0, 1).reshape(-1, feature_dim)).reshape(-1, seq_len, feature_dim).transpose(
                1, 0)
            r3 = self.normBNc(r3.transpose(0, 1).reshape(-1, feature_dim)).reshape(-1, seq_len, feature_dim).transpose(
                1, 0)
            r4 = self.normBNd(r4.transpose(0, 1).reshape(-1, feature_dim)).reshape(-1, seq_len, feature_dim).transpose(
                1, 0)
        ...
        if self.graph_type == 'GCN3' or self.graph_type == 'DeepGCN':
            ...
            emotions_feat, aware_features, spk_emb_vector, data, final_features = self.graph_model(features_a, features_v, features_l, seq_lengths, qmask, epoch)
            emotions_feat = self.dropout_(emotions_feat)
            emotions_feat = nn.ReLU()(emotions_feat)
            log_prob = F.log_softmax(self.smax_fc(emotions_feat), 1)
        else:
            print("There are no such kind of graph")
        return log_prob, edge_index, edge_norm, edge_type, edge_index_lengths, aware_features, spk_emb_vector, data, final_features
