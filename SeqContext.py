...
class SeqContext(nn.Module):
    def __init__(self, u_dim, g_dim, device):
        ...
        if self.rnn == "lstm":
            ...
            self.rnn = nn.LSTM(
                ...
                bidirectional=True,
                num_layers=self.seqcontext_nlayer,
                batch_first=True,
            )
        elif self.rnn == "gru":
            ...
            self.transformer_encoder = torch.nn.TransformerEncoder(
                encoder_layer, num_layers=self.seqcontext_nlayer
            )
            self.transformer_out = torch.nn.Linear(
                self.input_size, self.hidden_dim, bias=True
            )
            
            print("args.drop_rate:", self.drop_rate)
